...
def quiz():
    #퀴즈페이지 렌더링합니다
    #참여자 ID 쿠키가 필요합니다(다른사람의 퀴즈가 아니라서 참여자의 쿠키가 필요)
    if not participant_id: # type: ignore
    ...
    #render_template = 프로젝트 폴더 내에 template라는 폴더속의 html파일을 연결한다


@main.route("/submit", methods=["POST"])
def submit():
    # 참여자 ID가 필요합니다.
    participant_id = request.cookies.get("participant_id")      #참가자의 ID를 확인
    if not participant_id:                                      #참가자의 아이디가 없을 시 아래와 같은 오류코드를 보여줌
